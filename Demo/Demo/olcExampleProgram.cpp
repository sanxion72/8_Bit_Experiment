...
private: 
	...
	byte factMult;  // 1: res. 320 o 40 col. ; 2: res. 640 o 80 col. 
...
public:
	...
	enum ScreenMode { _MODE40_ = 1, _MODE80_ = 2, _MODEG320_ = 10, _MODEG640_ = 11 };
	ScreenMode myMode;

	void InitScreen(ScreenMode mode) {
		
		myMode = mode;
		
		if ((mode == _MODE40_) || (mode == _MODEG320_)) { colMode80 = false; factMult = 1; }
		if ((mode == _MODE80_) || (mode == _MODEG640_)) { colMode80 = true; factMult = 2; }
		
		environment.screenMode(mode);
		
		if (Construct(420 * factMult, 280, (colMode80 ? 1 : 2), 2))
			Start();
	}
	...
	bool OnUserCreate() override {
		...
		environment.screenMemIndex = 200 * factMult;
		environment.newScreenMemIndex = 200 * factMult;
	...
	bool OnUserUpdate(float fElapsedTime) override {
		...
		strAppo = "";
		if ((myMode == _MODE40_) || (myMode == _MODE80_)) {
			...
			if (IsFocused()) {
				...
				if (GetKey(olc::Key::UP).bPressed) {
					strAppo = "UP";
					if (environment.fTimeChar >= 0.06f) {
						if (environment.screenMemIndex > (colMode80 ? 79 : 39)) environment.newScreenMemIndex -= 40 * factMult;
						environment.fTimeChar = 0;
					}
				}
				...
				if (GetKey(olc::Key::DOWN).bPressed) {
					strAppo = "DOWN";
					if (environment.fTimeChar >= 0.06f) {
						if (environment.screenMemIndex < (colMode80 ? 2319 : 1159)) environment.newScreenMemIndex += 40 * factMult;
						environment.fTimeChar = 0;
					}
				}
				...
				if (GetKey(olc::Key::ENTER).bPressed) {
					strAppo = "ENTER";
					if (environment.fTimeChar >= 0.06f) {
						if (environment.screenMemIndex < (colMode80 ? 2319 : 1159)) environment.newScreenMemIndex = ((environment.cursorRow + 1) * 40 * factMult);
						environment.fTimeChar = 0;
					}
				}
			}
			...
			*/
			...
			// Gestisci cursore e timing di azzeramento per lampeggio
			environment.SetCursor(this, strAppo);
			if (environment.fTimeEl >= 0.6f) environment.fTimeEl = 0.0f;

			DrawString(50 * factMult, 260, "Tasto : " + strAppo);
			DrawString(170 * factMult, 260, "Tempo : " + tostr(environment.fTimeEl));
			DrawString(50 * factMult, 268, "Riga : " + tostr(environment.cursorRow) + "     Colonna : " + tostr(environment.cursorCol) + "        " + tostr(environment.screenMemIndex));
		}
...
int main()
{
	...
	demo.InitScreen(demo._MODEG320_);
